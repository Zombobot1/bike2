...
export function UChecksOptions({
  ...
  return (
    <Root sx={inline ? { display: 'inline-block', transform: 'translateY(-2px)' } : {}}>
      {inline && <FeedbackIndex i={i} validity={overallCorrectness} submitted={submitted} />}
      {options.map((o, i) => {
        let validity: Correctness = 'none'
        const ignoreValidationError = inline && answer.length
        if (error && !ignoreValidationError) validity = 'incorrect'
        else if (submitted) validity = optionValidity(o, answer, correctAnswer)
        return (
          <UChecksOption
